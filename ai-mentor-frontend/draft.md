...
**Таблиця 3 — Порівняння підходів до навчання програмуванню**
...
Запропонована методика базується на циклічній моделі: **теорія → практика → рефлексія**, де AI виступає підтримкою на кожному етапі (табл. 2).

**Таблиця 2 — Циклічна модель навчання з AI-ментором**

| Етап | Діяльність студента | Роль AI-ментора |
|------|--------------------|-----------------| 
| Теорія | Вивчає нову концепцію, ставить запитання | Пояснює, дає аналогії, адаптує складність, пояснює повторно під іншим кутом |
| Практика | Пише код самостійно, виконує задачі | Консультує, дає підказки, задає навідні запитання, НЕ пише код |
| Рефлексія | Показує результат, аналізує свій код | Дає зворотний зв'язок, вказує на проблеми, пропонує покращення |

*Цикл повторюється для кожної нової концепції або спринту.*
...
**Таблиця 5 — Спринтова прогресія навчальних завдань**

| Спринт | Тема | Ключові навички | Складність |
|:------:|------|-----------------|:----------:|
| 1 | Авторизація | Реєстрація, вхід, JWT-токени, захищені маршрути, валідація форм | Базова |
| 2 | Читання даних | Списки, профіль, обробка помилок, стани завантаження, робота з API | Середня |
| 3 | Повний CRUD | Створення, редагування, видалення, робота з файлами, пагінація | Підвищена |
| 4 | Розширення функціоналу | Пріоритети, фільтри, оновлення дизайну, оптимізація рендерингу | Висока |
...
**Таблиця 4 — Схема взаємодії студента з AI-ментором у Smart IDE**

| Компонент | Опис | Зв'язок з AI |
|-----------|------|--------------|
| Студент | Пише код, ставить запитання у чаті IDE | Діалог: запитання → відповідь |
| AI-ментор | Аналізує контекст, формує відповіді | Бачить усе середовище проекту |
| Проект (код) | Файли, компоненти, модулі студента | AI аналізує та дає рекомендації |
| Задачі спринту | Опис вимог, сценарії прийняття | AI знає поточну задачу студента |
| Помилки / логи | Повідомлення консолі, TypeScript-помилки | AI діагностує причину |
| Структура проекту | Директорії, залежності, конфігурація | AI враховує при відповідях |
